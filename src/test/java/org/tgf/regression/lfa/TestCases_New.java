...
public class TestCases_New extends GenericFunctions
{
	//TODO a variable to keep a track of which user is logged in.
	String curr_Logged_In_User; 
	
    Operations system_Admin    = new Operations();
    Operations lfa_Coordinator = new Operations();
    Operations ct_User         = new Operations();
    Operations portal_User     = new Operations();
	
	@BeforeClass
	public void Before_Class() throws Exception
	{
		system_Admin.open_Browser(CHROME);
		lfa_Coordinator.open_Browser(CHROME);
		ct_User.open_Browser(CHROME);
		portal_User.open_Browser(CHROME);
	}
